...
double screen_width_mm=0.6;           // 远处接收衍射图像屏幕的宽度，单位为mm
...
gsl_complex calculateComplexAmplitudeOneSlit(const double slit_position[], double x_screen, double time) {
    ...
    for(double i=slit_position[0];i<=slit_position[1];i+=(slit_position[1]-slit_position[0])/100) {
...
int main() {
    ...
    // 定义单缝的起始位置和终止位置
    double slit_position[2] = {-slit_width_mm/2.0*pow(10,-3),slit_width_mm/2.0*pow(10,-3)};
    // 计算光的一个周期的时间
    ...
    for(double y=-screen_width_mm/2.0; y<screen_width_mm/2.0; y+=integrate_step) {
        ...
        for(double time=0; time<period; time+=integrate_step) {
            light_intensity+=gsl_pow_int(GSL_REAL(calculateComplexAmplitudeOneSlit(slit_position,y,time)),2);
        }
